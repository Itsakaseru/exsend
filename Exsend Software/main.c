...
void load_database()
{
	...
	while ((fscanf(user_db, "%d#%[^\#]#%[^\n]", &user_id[user_count], &user[user_count], &password[user_count])) != EOF)
	{
		user_count++;
	}
	...
	while ((fscanf(order_db, "%d#%[^\#]#%[^\#]#%[^\#]#%[^\#]#%[^\#]#%[^\#]#%[^\#]#%d",
		&order_id[order_count], &order_status[order_count], &sender_name[order_count], &receiver_name[order_count], &sender_address[order_count], &pickup_location[order_count], &receive_address[order_count], &receive_location[order_count], &price_info[order_count])) != EOF)
	{
		order_count++;
	}
	...
	for (int i = 0; i < order_count; i++)
	{
		...
		printf("Receiver Address : %s", receive_address[i]); printf("\n");
		printf("Receiver location Location : %s", receive_location[i]); printf("\n");
...
void customer_delivery()
{
	...
	if(input_check >= 6)
    {
        ...
        getch();
        dijkstra();
        input_price = ( ( distance[destination] / 2 ) * 10000 );
    }

	printf("\t\t\t\t"); printf("1) Sender's name                : %s", input_sender_name);           printf("\n");
...
void delivery_name()
{
    switch(choices)
    {
        case 1 :
        {
            ...
            printf("\t\t\t\t\t"); printf("Sender's name                : ");  scanf("%[^\n]", input_sender_name); getchar();
...
void menu_courier()
{
	...
	printf("                                      1) EXSend delivery"); printf("\n");
...
void menu_order()
{
	...
	printf("                                      4) order address     : "); printf("\n");
	printf("\n");
	//nanti di sini mungkin pake function buat calculate price??
	printf("                                      order fees : "); printf("\n");
	printf("                                      order route  : "); printf("\n"); //ini kalau gamau gausah???
	printf("\n");
...
int main()
{
    ...
    for(int i = 0; i < node_count; i++)
    {
        printf("[%d] = %d", i, distance[i]); printf("\n");
    }
